...
class beacon_detection(t.Task):
    ...
    def step(self):
        ...
        for i in range(0, len(contours)):
            if m[i]['m00'] != 0:
                if cv2.contourArea(contours[i]) == area_max:
                    ...
                    cxi.append(int(m[i]['m10'] / m[i]['m00'] * cv2.contourArea(contours[i]) / total_area_n3))
            else:
                cxi.append(0)
        ...
        left_boundary = picture_size[1] / 6 - picture_size[1] * error_percent
        right_boundary = picture_size[1] / 6 + picture_size[1] * error_percent
        ...
        if total_area_n3 > 120000:
            # print 'need to stop'
            l, r = [0, 0]
            self.flag_beacon_end3 = True
        else:
