...
rec = t.get_recorder('color_tracing')
...
class ColorTracingTask(t.Task):
    ...
    def __init__(self):
        ...
        self.turning1 = Turning() #定角度转弯的任务

        self.turning2 = Turning()
        self.turn_over1 = False  #显示定角度转弯的任务是否完成
        self.flag_cfc_end = False
        self.count=0
    ...
    def step(self):
        ...
        if self.find_first_task.stop == 0: #如果第一个任务没有停止，则一直运行第一个任务
            ...
            if self.find_second_task.color == boundaries_red:
                # 如果色块为红色
                # 逆时针旋转45度
                self.turning1.want_degree = 45
                if self.turn_over1 == False:
                    while self.turn_over1 == False:
                        self.turn_over1 = self.turning1.step()
                    return
                if self.turn_pause() == 1:
                    return
                self.find_second_task.step()#运行找色块任务
                if self.find_second_task.stop == 1:
                    self.turning2.want_degree = 70
                    if self.flag_cfc_end== False:
                        while self.flag_cfc_end == False:
                            self.flag_cfc_end = self.turning2.step()
                        return
                    return
            ...
            # if self.count < 10:
            #     with rec.a_group():
            #         rec.record_img('img', self.find_second_task.im)
            #         rec.record_plain('turning', self.turn_over)
    ...
    def turn_pause(self):
        if self.count < 5:
            self.count += 1
            self.find_second_task.speed=0
            self.find_second_task.direction=0
            t.peripheral.wheels.set_diff(self.find_second_task.speed, -self.find_second_task.direction)
            return 1
        else:
            return 0
...
class FindBlockTask(t.Task):
    ...
    def step(self):
        ...
        # cv2.destroyAllWindows()
        if self.stop == 1:
            self.stop = ct.stop_check(area, self.stop)
            print 'toroise stop'
            self.speed = 0
            self.direction = 0
            ...
            if self.found == False:
                ...
                self.stop = ct.stop_check(area,self.stop)
                print 'max area_list is', max(ct.area_list)
                print self.color
